...
$delay = '@6us';
# set up the output port so that packets from CPU ports can be forwarded to PHY ports
for($i=0; $i<8; $i++) {
  nf_PCI_write32($delay, 0, HARDWIRE_LOOKUP_OUTPUT_PORTS_BASE_REG()+$i*4, 1<<(($i+1)%8));
}
...
for($i=0; $i<4; $i++) {
  ...
  $vlan_tag_out[$i] = int(rand(4093)) + 1;
}
...
# Set the values for VLAN tag modification
nf_PCI_write32($delay, 0, VLAN_ADDER_0_VLAN_TAG_REG(), $vlan_tag_out[0]);
nf_PCI_write32($delay, 0, VLAN_ADDER_1_VLAN_TAG_REG(), $vlan_tag_out[1]);
nf_PCI_write32($delay, 0, VLAN_ADDER_2_VLAN_TAG_REG(), $vlan_tag_out[2]);
nf_PCI_write32($delay, 0, VLAN_ADDER_3_VLAN_TAG_REG(), $vlan_tag_out[3]);
# verify the values above
nf_PCI_read32($delay, 0, VLAN_ADDER_0_VLAN_TAG_REG(), $vlan_tag_out[0]);
nf_PCI_read32($delay, 0, VLAN_ADDER_1_VLAN_TAG_REG(), $vlan_tag_out[1]);
nf_PCI_read32($delay, 0, VLAN_ADDER_2_VLAN_TAG_REG(), $vlan_tag_out[2]);
nf_PCI_read32($delay, 0, VLAN_ADDER_3_VLAN_TAG_REG(), $vlan_tag_out[3]);

# Specify physical output port
...
for($i=0; $i<4; $i++){
  ...
  for($j=0; $j<15; $j++){
    ...
    $exp_pkt = make_vlan_pkt($length, $DA, $SA, $vlan_tag_out[($i+1)%4]);

    nf_dma_data_in($length, 0, $in_port, $pkt);
    nf_expected_packet($exp_port, $length+4, $exp_pkt);
  }
  $delay = $delay_us[$i];
}
