...
#include "../lib/C/reg_defines_anti_spoof.h"
...
void print(void) {
	...
	printf("anti spoof MAC lut num hits:                   %u\n", val);
	readReg(&nf2, ANTI_SPOOF_AS_NUM_MISSES_REG, &val);
	printf("anti spoof MAC lut num misses:                 %u\n\n", val);
	for(i=0; i<16; i=i+1){
	  writeReg(&nf2, ANTI_SPOOF_AS_MAC_LUT_RD_ADDR_REG, i);
	  readReg(&nf2, ANTI_SPOOF_AS_PORTS_MAC_HI_REG, &val);
	  printf("   Anti spoof CAM table entry %02u: wr_protect: %u, ports: 0x%04x, mac: 0x%04x", i, val>>31, (val&0x7fff0000)>>16, (val&0xffff));
	  readReg(&nf2, ANTI_SPOOF_AS_MAC_LO_REG, &val);
	  printf("%08x\n", val);
	}

	readReg(&nf2, SWITCH_OP_LUT_NUM_HITS_REG, &val);
	printf("switch MAC lut num hits:                   %u\n", val);
	readReg(&nf2, SWITCH_OP_LUT_NUM_MISSES_REG, &val);
	printf("switch MAC lut num misses:                 %u\n\n", val);
	for(i=0; i<16; i=i+1){
	  writeReg(&nf2, SWITCH_OP_LUT_MAC_LUT_RD_ADDR_REG, i);
	  readReg(&nf2, SWITCH_OP_LUT_PORTS_MAC_HI_REG, &val);
	  printf("   switch CAM table entry %02u: wr_protect: %u, ports: 0x%04x, mac: 0x%04x", i, val>>31, (val&0x7fff0000)>>16, (val&0xffff));
