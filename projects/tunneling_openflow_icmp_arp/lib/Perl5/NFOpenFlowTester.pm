...
sub make_ICMP_pkt{ # len, DA, SA, TTL, DST_IP, SRC_IP, ICMP_TYPE, ICMP_CODE, ICMP_ID, ICMP_SEQ

  my ($len, $DA, $SA, $TTL, $DST_IP, $SRC_IP, $ICMP_TYPE, $ICMP_CODE, $ICMP_ID, $ICMP_SEQ) = @_;
  ...
  my $ICMP_hdr = NF::ICMP->new(      Type => $ICMP_TYPE,
                                     Code => $ICMP_CODE,
                                     Checksum => 0x0,
                                     Identifier => $ICMP_ID,
                                     SeqNo => $ICMP_SEQ
  ...
  $ICMP_hdr->Checksum(0);  # make sure its zero before we calculate it.
  $ICMP_hdr->Checksum($ICMP_hdr->calc_checksum);

  # create packet filling.... (IP PDU)
  my $PDU = NF::PDU->new($len - $MAC_hdr->length_in_bytes() - $IP_hdr->length_in_bytes() - $ICMP_hdr->length_in_bytes());
  my $start_val = $MAC_hdr->length_in_bytes() + $IP_hdr->length_in_bytes() + $ICMP_hdr->length_in_bytes()+1;
  my @data = ($start_val..$len);
  for (@data) {$_ %= 100}
  $PDU->set_bytes(@data);

  # Return complete packet data
  $MAC_hdr->bytes().$IP_hdr->bytes().$ICMP_hdr->bytes().$PDU->bytes();
}
