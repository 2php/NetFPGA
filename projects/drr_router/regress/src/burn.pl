...
my @counters_to_clear = (
	'RX_QUEUE_X_NUM_PKTS_STORED_REG()',
	'RX_QUEUE_X_NUM_PKTS_DROPPED_FULL_REG()',
	'RX_QUEUE_X_NUM_PKTS_DROPPED_BAD_REG()',
	'RX_QUEUE_X_NUM_WORDS_PUSHED_REG()',
	'RX_QUEUE_X_NUM_BYTES_PUSHED_REG()',
	'RX_QUEUE_X_NUM_PKTS_DEQUEUED_REG()',
	'RX_QUEUE_X_NUM_PKTS_IN_QUEUE_REG()',
	'TX_QUEUE_X_NUM_PKTS_IN_QUEUE_REG()',
	'TX_QUEUE_X_NUM_PKTS_SENT_REG()',
	'TX_QUEUE_X_NUM_WORDS_PUSHED_REG()',
	'TX_QUEUE_X_NUM_BYTES_PUSHED_REG()', 
	'TX_QUEUE_X_NUM_PKTS_ENQUEUED_REG()'
);
...
#	$PDU->set_bytes(@data);
	...
	my $MAC_hdr2 = NF2::Ethernet_hdr->new(DA => get_dest_MAC(0),
						     SA => get_dest_MAC(0),
						     Ethertype => 0x800
				    		);
...
my %reg_expected_hash = (
	'"RX_QUEUE_${i}_NUM_PKTS_STORED_REG()"' =>
		'"($packets_to_loop * $packets_sent[$p]) % NUM_2_32"',
	'"RX_QUEUE_${i}_NUM_PKTS_DROPPED_FULL_REG()"' =>
		'"0"',	
	'"RX_QUEUE_${i}_NUM_PKTS_DROPPED_BAD_REG()"' =>
		'"0"',
	'"RX_QUEUE_${i}_NUM_WORDS_PUSHED_REG()"' =>
		'"($packets_to_loop * $packets_sent[$p] * (($len >> 3) << 3) / 8) % NUM_2_32"',
	'"RX_QUEUE_${i}_NUM_BYTES_PUSHED_REG()"' =>
		'"($packets_to_loop * $packets_sent[$p] * $len) % NUM_2_32"',
	'"RX_QUEUE_${i}_NUM_PKTS_DEQUEUED_REG()"' =>
		'"($packets_to_loop * $packets_sent[$p]) % NUM_2_32"',
	'"RX_QUEUE_${i}_NUM_PKTS_IN_QUEUE_REG()"' =>
		'"0"',	
	'"TX_QUEUE_${i}_NUM_PKTS_IN_QUEUE_REG()"' =>
		'"0"',	
	'"TX_QUEUE_${i}_NUM_PKTS_SENT_REG()"' =>
		'"($packets_to_loop * $packets_sent[$i]) % NUM_2_32"',
	'"TX_QUEUE_${i}_NUM_WORDS_PUSHED_REG()"' =>
		'"($packets_to_loop * $packets_sent[$i] * (($len >> 3) << 3) / 8) % NUM_2_32"',
	'"TX_QUEUE_${i}_NUM_BYTES_PUSHED_REG()"' =>
		'"($packets_to_loop * $packets_sent[$i] * $len) % NUM_2_32"',
	'"TX_QUEUE_${i}_NUM_PKTS_ENQUEUED_REG()"' =>
		'"($packets_to_loop * $packets_sent[$i]) % NUM_2_32"'
);
...
for (my $i = 0; $i < $num_ports; $i++) {
	...
	my $p = get_transmitting_port($i);
	my $reg = eval "RX_QUEUE_${i}_NUM_PKTS_STORED_REG()";
...
sub get_q_num_pkts_reg {
	my $i = shift;
	if ($i == 0) { return OQ_NUM_PKTS_IN_Q_REG_0(); }
	if ($i == 1) { return OQ_NUM_PKTS_IN_Q_REG_2(); }
	if ($i == 2) { return OQ_NUM_PKTS_IN_Q_REG_4(); }
	if ($i == 3) { return OQ_NUM_PKTS_IN_Q_REG_6(); }	
}
