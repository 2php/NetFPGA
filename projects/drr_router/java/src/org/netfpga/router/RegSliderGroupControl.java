...
public class RegSliderGroupControl {
    ...
    public RegSliderGroupControl(NFDevice nf2, JSlider slider, JLabel valueLabel, long address1, long address2,
                                 ValueTransformer vt) {
        long addr[] = {address1,address2};
        this.regTableModel = new RegTableModel(nf2, addr);
        this.slider = slider;
        this.valueLabel = valueLabel;
        
        /* We need to divide the value of address1 by the value of address2. */
        int val = 10 * (Integer)regTableModel.getValueAt(0, RegTableModel.VALUE_COL) / (Integer)regTableModel.getValueAt(1, RegTableModel.VALUE_COL);
    ...
    public RegSliderGroupControl(NFDevice nf2, JSlider slider, JLabel valueLabel, long address1, long address2) {
        this(nf2,slider,valueLabel,address1,address2,null);
    }
    ...
    public void updateFromTwoRegs() {
        if(!slider.getValueIsAdjusting()){
            try {
                int val = (int)(10f * new Float((Integer)regTableModel.getValueAt(0, RegTableModel.VALUE_COL)) / new Float((Integer)regTableModel.getValueAt(1, RegTableModel.VALUE_COL)));
                System.out.println("Setting slider to "+vt.toSliderValue(val)+" from regs");
                valueLabel.setText(vt.toLabelStringFromReg(val));
                slider.setValue(vt.toSliderValue(val));
            } catch(ArithmeticException ex) {
                /* TODO: is setting to 0 correct here? */
